...
const deleteJob = async (job_id) => {
  try {
    if (confirm("Are you sur you want to delete this job ?")) {
      const response = await axios.delete(`/api/jobs/${job_id}`)
      toast.success("Job deleted successfully !");
      router.push('/jobs')
    } else {
      toast.warning("Action aborted successfully");
    }
  } catch (error) {
    console.error("Something went wrong in deleting a job", error)
    toast.error(error)
  }
}
      ...
      <div class="grid grid-cols-1 md:grid-cols-70/30 w-full gap-6">
        <main>
          <div class="bg-white p-6 rounded-lg shadow-md text-center md:text-left">
            ...
            <div class="text-gray-500 mb-4 flex items-center align-middle justify-center md:justify-start">
